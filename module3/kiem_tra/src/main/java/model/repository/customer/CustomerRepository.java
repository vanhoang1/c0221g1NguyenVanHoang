...
public class CustomerRepository extends BaseRepository<Customer> {
    ...
    public CustomerRepository() {
        ...
        this.INSERT= "INSERT INTO `case_study`.`khach_hang` (`id_loai_khach`, `ho_ten`, `ngay_sinh`, gioi_tinh, `so_cmtdn`, `sdt`,ma_khach_hang, `email`, `dia_chi`) VALUES (?, ? ,?, ?, ?, ?, ?, ?,?);";
        this.UPDATE="UPDATE `khach_hang` SET `id_loai_khach` = ?, `ho_ten` = ?, `ngay_sinh` = ?, gioi_tinh=? , `so_cmtdn` = ? ,`sdt` = ?,ma_khach_hang=?, `email` = ?, `dia_chi` = ? WHERE (`id_khach_hang` = ?);";
        this.ID=10;
    ...
    public Customer get(ResultSet resultSet) {
        try {
            ...
            return new Customer(id, nameCustomer, dateOfBirth, soCMND, gender, soDT,maKhachHang, address, email, customerType);
    ...
    public void set(Customer customer, PreparedStatement preparedStatement) {
        try {
            ...
            preparedStatement.setString(7, customer.getMaKhachHang());
