        ...
        <div class="mx-auto ">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered " id="tableList">
                    ...
                    <c:forEach items='${list}' var="summary" varStatus="count">
                        <tr >
                            <td class="text-center">${count.count}</td>

                            <td>${summary.getContract().getCustomer().getNameCustomer()}</td>
                            <td>${summary.getContract().getCustomer().getDateOfBirth()}</td>
                            <td>${summary.getContract().getCustomer().getSoDT()}</td>
                            <td>${summary.getContract().getCustomer().getAddress()}</td>
                            <td>${summary.getContract().getCustomer().getEmail()}</td>
                            <td><a href="/services?action=edit&id=${summary.getContract().getService().getId()}">
                                    ${summary.getContract().getService().getServiceName()}</a>
                            </td>
                            <td >${summary.getContractDetail().getAttachService().getAttachName()}</td>
                            <td class="text-center">${summary.getContractDetail().getQuantity()}</td>


                                <%--                        thao tac--%>
                            <td class="td-actions text-right">

                                <a href="" class="text-light"> <button type="button" rel="tooltip" class="btn btn-info btn-round btn-just-icon btn-sm"
                                                                                                                  data-original-title="" title="info"> <i
                                        class="fa fa-user"></i> </button> </a>


                                <a href=" " class="text-light"> <button type="button" rel="tooltip"
