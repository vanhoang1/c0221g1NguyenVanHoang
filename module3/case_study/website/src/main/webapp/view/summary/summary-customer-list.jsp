                ...
                <table class="table table-striped table-bordered " id="tableList">
                    ...
                    <c:forEach items='${list}' var="summary" varStatus="count">
                        <tr >
                            ...
                            <td>${summary.contract.customer.nameCustomer}</td>
                            <td>${summary.contract.customer.dateOfBirth}</td>
                            <td>${summary.contract.customer.soDT}</td>
                            <td>${summary.contract.customer.address}</td>
                            <td>${summary.contract.customer.email}</td>
                            <td>${summary.contract.service.serviceName}</td>
                            <td >${summary.contractDetail.attachService.attachName}</td>
                            <td class="text-center">${summary.contractDetail.quantity}</td>


                                <%--                        thao tac--%>
                            <td class="td-actions text-right">

                                <a href="/contracts?action=edit&id=${summary.contract.idContract}" class="text-light"> <button type="button" rel="tooltip" class="btn btn-info btn-round btn-just-icon btn-sm"
                                                                                                                  data-original-title="" title="info"> <i
                                        class="fa fa-user"></i> </button> </a>


                                <a href=" /contracts?action=edit&id=${summary.contract.idContract}" class="text-light"> <button type="button" rel="tooltip"
